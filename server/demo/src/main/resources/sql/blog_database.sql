...
('hot_search_count', '10', 'number', '热门搜索显示数量');
...
1. 用户系统:
   - users: 用户基本信息
   - user_badges: 用户徽章(一对多)
   - user_follows: 用户关注关系(多对多)
...
- 为常用查询字段添加了索引
- 文章表添加了全文索引支持搜索
- 使用冗余字段(如post_count)减少JOIN查询

注意事项:
- 使用UTF8MB4字符集支持emoji
- 软删除使用deleted_at字段
- 时间戳使用DATETIME类型
- IP地址支持IPv6(VARCHAR(45))
*/
