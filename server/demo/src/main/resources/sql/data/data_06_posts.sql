...
550, 3, 1543, 112, 19, 1, 1, 0, NOW(), NOW(), NOW()) ON DUPLICATE KEY UPDATE id=id;
...
500, 2, 1234, 87, 11, 1, 1, 0, NOW(), NOW(), NOW()) ON DUPLICATE KEY UPDATE id=id;
...
480, 2, 1987, 145, 28, 1, 1, 0, NOW(), NOW(), NOW()) ON DUPLICATE KEY UPDATE id=id;
...
480, 2, 1234, 89, 14, 1, 1, 0, NOW(), NOW(), NOW()) ON DUPLICATE KEY UPDATE id=id;
...
480, 2, 1876, 134, 22, 1, 1, 0, NOW(), NOW(), NOW()) ON DUPLICATE KEY UPDATE id=id;
...
520, 3, 1654, 143, 27, 1, 1, 0, NOW(), NOW(), NOW()) ON DUPLICATE KEY UPDATE id=id;


-- 追加样例
INSERT INTO posts (user_id, category_id, title, slug, excerpt, content, content_html, word_count, read_time, view_count, like_count, comment_count, status, is_original, is_featured, allow_comment, published_at, created_at, updated_at) VALUES
(13, 7, 'Flutter 性能优化实战', 'flutter-performance-tips',
'整理 Flutter 列表与渲染性能优化要点。',
'# Flutter 性能优化实战
...
## 小结
从列表、渲染、资源加载三个维度优化体验。',
'<h1>Flutter 性能优化实战</h1><p>核心优化点与实践建议。</p>',
420, 2, 120, 6, 2, 2, 1, 1, 0, NOW(), NOW(), NOW()),
(14, 8, 'Web 安全入门与实践', 'web-security-basics',
'涵盖 XSS、CSRF、SQL 注入的基础防护要点。',
'# Web 安全入门与实践
...
## 建议
开启 CSP，使用 HttpOnly Cookie。',
'<h1>Web 安全入门与实践</h1><p>基础安全概念与防护清单。</p>',
380, 2, 80, 4, 1, 1, 1, 1, 0, NOW(), NOW(), NOW())
